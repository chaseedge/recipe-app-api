...
class Public(TestCase):
    ...
    def test_user_exists(self):
        ...
        create_user(**payload)  # creates the first user

        res = self.client.post(CREATE_USER_URL, payload)  # creates user again
    ...
    def test_create_token_for_user(self):
        ...
        # makes sure there's a key named token in data
        self.assertIn('token', res.data)
        self.assertEqual(res.status_code, status.HTTP_200_OK)

    def test_create_token_invalid(self):
        """Test that a token isn't created when user credentials are invalid"""
